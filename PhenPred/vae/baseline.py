...
if __name__ == "__main__":
        ...
        def __init__(self, views, hidden_dim, latent_dim, probability, group, activation_function) -> None:
            ...
            for v in views:
                self.omics_decoders.append(
                    nn.Sequential(
                        nn.Linear(latent_dim, v.shape[1]),
                    )
                )
    ...
    def mse_kl(views_hat, views, mu_joint, logvar_joint, means, log_variances, loss_type, alpha=0.1, lambd=1.0):
    ...
    def plot_losses(losses_dict,losses_datasets, alpha_KL, alpha_MSE):
        ...
        ax = sns.lineplot(data=pd.melt(losses.loc[:,["loss_train","loss_val","epoch"]], ['epoch']), x="epoch", y="value", hue="variable")
        ax.set(title= f'Train and Validation Loss (lambda = {alpha_MSE} , alpha = {alpha_KL})', xlabel='Epoch', ylabel='Train Loss and Validation Loss')
        legend_handles, _ = ax.get_legend_handles_labels()
        ax.legend(legend_handles,["Train Loss","Validation Loss"], title = "Losses")
        ax.xaxis.set_major_locator(MaxNLocator(integer=True))
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/losses/{dt}train_validation_loss_.pdf', dpi=300, bbox_inches="tight") 
        plt.close()
        ...
        sns.set_theme()
        sns.set_context("paper")
        ax = sns.lineplot(data=pd.melt(losses.loc[:,["mse_train","kl_train","epoch"]], ['epoch']), x="epoch", y="value", hue="variable")
        ax.set(title = f'Total loss (lambda = {alpha_MSE} , alpha = {alpha_KL})',xlabel='Epoch', ylabel='Reconstruction Loss and Regularization Loss')
        legend_handles, _ = ax.get_legend_handles_labels()
        ax.legend(legend_handles,["Reconstruction Loss","KL Loss"], title = "Losses")
        ax.xaxis.set_major_locator(MaxNLocator(integer=True))
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/losses/{dt}reconst_reg_loss_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        sns.set_theme()
        sns.set_context("paper")
        ax = sns.lineplot(data=pd.melt(losses_omics, ['epoch']), x="epoch", y="value", hue="variable")
        ax.set(xlabel='Epoch', ylabel='Reconstruction Loss for each dataset')
        legend_handles, _ = ax.get_legend_handles_labels()
        ax.legend(legend_handles,["Transcriptomics","Proteomics","Drug Response","CRISPR","Metabolomics","Methylation"], title = "Reconstruction Losses")
        ax.xaxis.set_major_locator(MaxNLocator(integer=True))
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/losses/{dt}reconst_omics_loss_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 


    def plot_latent_spaces(latent_space, latent_space_transcriptomics, latent_space_proteomics, latent_space_drugresponse, latent_space_crisprcas9, latent_space_metabolomics, latent_space_methylation, hidden_dim, latent_dim, probability, group, learning_rate, n_folds, batch_size_initial):
        ...
        latent_space_umap = pd.DataFrame(umap.UMAP(n_neighbors=15, min_dist=0.1, metric="euclidean").fit_transform(latent_space))
        latent_space_transcriptomics_umap = pd.DataFrame(umap.UMAP(n_neighbors=15, min_dist=0.1, metric="euclidean").fit_transform(latent_space_transcriptomics))
        latent_space_proteomics_umap = pd.DataFrame(umap.UMAP(n_neighbors=15, min_dist=0.1, metric="euclidean").fit_transform(latent_space_proteomics))
        latent_space_drugresponse_umap = pd.DataFrame(umap.UMAP(n_neighbors=15, min_dist=0.1, metric="euclidean").fit_transform(latent_space_drugresponse))
        latent_space_crisprcas9_umap = pd.DataFrame(umap.UMAP(n_neighbors=15, min_dist=0.1, metric="euclidean").fit_transform(latent_space_crisprcas9))
        latent_space_metabolomics_umap = pd.DataFrame(umap.UMAP(n_neighbors=15, min_dist=0.1, metric="euclidean").fit_transform(latent_space_metabolomics))
        latent_space_methylation_umap = pd.DataFrame(umap.UMAP(n_neighbors=15, min_dist=0.1, metric="euclidean").fit_transform(latent_space_methylation))
        ...
        sns.set_theme()
        sns.set_style("dark")
        ax = sns.scatterplot(data=latent_space_umap, x="latent_1", y="latent_2", hue = "Tissue Type", palette = PALETTE_TTYPE, alpha=0.5)
        ax.set(title=f'UMAP projection of Joint Latent Space \nhidden_dim = {hidden_dim}, latent_dim = {latent_dim}, dropout = {probability}, n_groups = {group},\nlr = {learning_rate}, n_folds = {n_folds}, batch_size = {batch_size}', xlabel='UMAP_1', ylabel='UMAP_2', xticklabels=[], yticklabels=[])
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/latent/{dt}umap_joint_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        sns.set_theme()
        sns.set_style("dark")
        ax = sns.scatterplot(data=latent_space_umap, x="latent_1", y="latent_2", c = transcriptomics.T.reindex(index=omics_db.samples)["VIM"].values, cmap= "viridis" , alpha=0.5)
        ax.set(title=f'UMAP projection of Joint Latent Space \nhidden_dim = {hidden_dim}, latent_dim = {latent_dim}, dropout = {probability}, n_groups = {group},\nlr = {learning_rate}, n_folds = {n_folds}, batch_size = {batch_size}', xlabel='UMAP_1', ylabel='UMAP_2', xticklabels=[], yticklabels=[])
        norm = plt.Normalize(transcriptomics.T.reindex(index=omics_db.samples)["VIM"].values.min(), transcriptomics.T.reindex(index=omics_db.samples)["VIM"].values.max())
        sm = plt.cm.ScalarMappable(cmap="viridis", norm=norm)
        sm.set_array([])
        ax.figure.colorbar(sm)
        plt.savefig(f'/home/sofiaapolinario/test/plots/latent/{dt}umap_joint_VIM_.pdf', dpi=300, bbox_inches="tight")
        plt.close() 
        ...
        sns.set_theme()
        sns.set_style("dark")
        ax = sns.scatterplot(data=latent_space_umap, x="latent_1", y="latent_2", c = metabolomics.T.reindex(index=omics_db.samples)["1-methylnicotinamide"].values, cmap= "viridis" , alpha=0.5)
        ax.set(title=f'UMAP projection of Joint Latent Space \nhidden_dim = {hidden_dim}, latent_dim = {latent_dim}, dropout = {probability}, n_groups = {group},\nlr = {learning_rate}, n_folds = {n_folds}, batch_size = {batch_size}', xlabel='UMAP_1', ylabel='UMAP_2', xticklabels=[], yticklabels=[])
        norm = plt.Normalize(metabolomics.T.reindex(index=omics_db.samples)["1-methylnicotinamide"].values.min(), metabolomics.T.reindex(index=omics_db.samples)["1-methylnicotinamide"].values.max())
        sm = plt.cm.ScalarMappable(cmap="viridis", norm=norm)
        sm.set_array([])
        ax.figure.colorbar(sm)
        plt.savefig(f'/home/sofiaapolinario/test/plots/latent/{dt}umap_joint_methy_.pdf', dpi=300, bbox_inches="tight")
        plt.close() 
        ...
        latent_space_transcriptomics_umap.columns = ["latent_1","latent_2","Tissue Type"]

        sns.set()
        sns.set_theme()
        sns.set_style("dark")
        ax = sns.scatterplot(data=latent_space_transcriptomics_umap, x="latent_1", y="latent_2", hue = "Tissue Type", palette = PALETTE_TTYPE, alpha=0.5)
        ax.set(title=f'UMAP projection of Transcriptomics Latent Space \nhidden_dim = {hidden_dim}, latent_dim = {latent_dim}, dropout = {probability}, n_groups = {group},\nlr = {learning_rate}, n_folds = {n_folds}, batch_size = {batch_size}', xlabel='UMAP_1', ylabel='UMAP_2', xticklabels=[], yticklabels=[])
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/latent/{dt}umap_transcriptomics_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        sns.set_theme()
        sns.set_style("dark")
        ax = sns.scatterplot(data=latent_space_proteomics_umap, x="latent_1", y="latent_2", hue = "Tissue Type", palette = PALETTE_TTYPE, alpha=0.5)
        ax.set(title=f'UMAP projection of Proteomics Latent Space \nhidden_dim = {hidden_dim}, latent_dim = {latent_dim}, dropout = {probability}, n_groups = {group},\nlr = {learning_rate}, n_folds = {n_folds}, batch_size = {batch_size}', xlabel='UMAP_1', ylabel='UMAP_2', xticklabels=[], yticklabels=[])
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/latent/{dt}umap_proteomics_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        sns.set_theme()
        sns.set_style("dark")
        ax = sns.scatterplot(data=latent_space_drugresponse_umap, x="latent_1", y="latent_2", hue = "Tissue Type", palette = PALETTE_TTYPE, alpha=0.5)
        ax.set(title=f'UMAP projection of Drug Response Latent Space \nhidden_dim = {hidden_dim}, latent_dim = {latent_dim}, dropout = {probability}, n_groups = {group},\nlr = {learning_rate}, n_folds = {n_folds}, batch_size = {batch_size}', xlabel='UMAP_1', ylabel='UMAP_2', xticklabels=[], yticklabels=[])
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/latent/{dt}umap_drugresponse_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        sns.set_theme()
        sns.set_style("dark")
        ax = sns.scatterplot(data=latent_space_crisprcas9_umap, x="latent_1", y="latent_2", hue = "Tissue Type", palette = PALETTE_TTYPE, alpha=0.5)
        ax.set(title=f'UMAP projection of CRISPR Latent Space \nhidden_dim = {hidden_dim}, latent_dim = {latent_dim}, dropout = {probability}, n_groups = {group},\nlr = {learning_rate}, n_folds = {n_folds}, batch_size = {batch_size}', xlabel='UMAP_1', ylabel='UMAP_2', xticklabels=[], yticklabels=[])
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/latent/{dt}umap_crispr_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        sns.set_theme()
        sns.set_style("dark")
        ax = sns.scatterplot(data=latent_space_metabolomics_umap, x="latent_1", y="latent_2", hue = "Tissue Type", palette = PALETTE_TTYPE, alpha=0.5)
        ax.set(title=f'UMAP projection of Metabolomics Latent Space \nhidden_dim = {hidden_dim}, latent_dim = {latent_dim}, dropout = {probability}, n_groups = {group},\nlr = {learning_rate}, n_folds = {n_folds}, batch_size = {batch_size}', xlabel='UMAP_1', ylabel='UMAP_2', xticklabels=[], yticklabels=[])
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/latent/{dt}umap_metabolomics_.pdf', dpi=300, bbox_inches="tight")
        plt.close()  
        ...
        sns.set_theme()
        sns.set_style("dark")
        ax = sns.scatterplot(data=latent_space_methylation_umap, x="latent_1", y="latent_2", hue = "Tissue Type", palette = PALETTE_TTYPE, alpha=0.5)
        ax.set(title=f'UMAP projection of Methylation Latent Space \nhidden_dim = {hidden_dim}, latent_dim = {latent_dim}, dropout = {probability}, n_groups = {group},\nlr = {learning_rate}, n_folds = {n_folds}, batch_size = {batch_size}', xlabel='UMAP_1', ylabel='UMAP_2', xticklabels=[], yticklabels=[])
        sns.move_legend(ax, "upper left", bbox_to_anchor=(1, 1))
        plt.savefig(f'/home/sofiaapolinario/test/plots/latent/{dt}umap_methylation_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
    ...
    def evaluate_drug_response(drugresponse_final):    
        ...
        #Autoencoder Dataset
        df_imputed = pd.DataFrame(omics_db.scaler_drug.inverse_transform(drugresponse_final))
        ...
        #Mofa Dataset
        df_imputed_mofa = pd.read_csv("/home/sofiaapolinario/test/dataMOFA/drugresponseMOFA.csv")
        ...
        #New Lab Values
        drug_response_new = pd.read_csv("/home/sofiaapolinario/test/dataMOFA/drugresponse_24Jul22.csv")
        drug_response_new["id"] = drug_response_new['DRUG_ID'].astype(str) +";"+ drug_response_new["DRUG_NAME"] +";"+ drug_response_new["DATASET"]
        drug_response_new = drug_response_new.drop(columns = ['DRUG_ID',"DRUG_NAME","DATASET"])
        ...
        #Correlation
        drugresponse_corr = pd.merge(drug_response_new_table, drugresponse_imputed_table,  how='outer', left_on=['id','variable'], right_on = ['id','variable'])
        drugresponse_corr.columns = drugresponse_corr.columns.str.replace('value_x', 'NEW')
        drugresponse_corr.columns = drugresponse_corr.columns.str.replace('value_y', 'IMPUTED')
        drugresponse_corr = pd.merge(drugresponse_corr, drug_response_table,  how='outer', left_on=['id','variable'], right_on = ['id','variable'])
        drugresponse_corr.columns = drugresponse_corr.columns.str.replace('value', 'ORIGINAL')
        drugresponse_corr = pd.merge(drugresponse_corr, drugresponse_imputed_mean_table,  how='outer', left_on=['id','variable'], right_on = ['id','variable'])
        drugresponse_corr.columns = drugresponse_corr.columns.str.replace('value', 'MEAN')
        drugresponse_corr = pd.merge(drugresponse_corr, drugresponse_imputed_mofa_table,  how='outer', left_on=['id','variable'], right_on = ['id','variable'])
        drugresponse_corr.columns = drugresponse_corr.columns.str.replace('value', 'MOFA')

        corr = drugresponse_corr[(~drugresponse_corr['NEW'].isna()) & (~drugresponse_corr['IMPUTED'].isna()) & (~drugresponse_corr['MOFA'].isna()) & (drugresponse_corr['ORIGINAL'].isna())]
        ...
        corr_mofa_VAE = pd.DataFrame(corr[["IMPUTED","MOFA"]].corr(method ='pearson')).iloc[0, 1]
        
        sns.set()
        sns.set_theme()
        sns.set_context("paper")
        ax = sns.scatterplot(data=corr, x="NEW", y="IMPUTED", alpha=0.5, color='green')
        ax.set(title='Correlation between New Measured Values and Autoencoder Predictions', xlabel='New Measured Values', ylabel='Autoencoder Predictions')
        X_plot = np.linspace(-8, 13, 20)
        Y_plot = X_plot
        plt.plot(X_plot, Y_plot, color='r')
        indices = np.where(np.isfinite(corr['NEW'].values) & np.isfinite(corr['IMPUTED'].values))[0]
        r, p = stats.pearsonr(x = corr['NEW'].iloc[indices].values, y = corr['IMPUTED'].iloc[indices].values)
        plt.text(.05, .8, 'personr = {:.4f}'.format(r), transform=ax.transAxes)
        plt.text(.05, .75, 'p-value = {:.4f}'.format(p), transform=ax.transAxes)
        plt.savefig(f'/home/sofiaapolinario/test/plots/drugresponse/{dt}drugresponse_new_imputed_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        ax.set(title='Correlation between New Measured Values and Autoencoder Predictions', xlabel='New Measured Values', ylabel='Autoencoder Predictions')
        plt.savefig(f'/home/sofiaapolinario/test/plots/drugresponse/{dt}drugresponse_new_imputed_density_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        ax.set(title='Correlation between MOFA and Autoencoder Predictions', xlabel='MOFA Predictions', ylabel='Autoencoder Predictions')
        X_plot = np.linspace(-8, 13, 20)
        Y_plot = X_plot
        plt.plot(X_plot, Y_plot, color='r')
        indices = np.where(np.isfinite(corr['MOFA'].values) & np.isfinite(corr['IMPUTED'].values))[0]
        r, p = stats.pearsonr(x = corr['MOFA'].iloc[indices].values, y = corr['IMPUTED'].iloc[indices].values)
        plt.text(.05, .8, 'personr = {:.4f}'.format(r), transform=ax.transAxes)
        plt.text(.05, .75, 'p-value = {:.4f}'.format(p), transform=ax.transAxes)
        plt.savefig(f'/home/sofiaapolinario/test/plots/drugresponse/{dt}drugresponse_mofa_imputed_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
    ...
    def evaluate_proteomics(proteomics_final): 
        ...
        # Imputed All Dataset
        df_imputed_all = pd.DataFrame(omics_db.scaler_prot.inverse_transform(proteomics_final))
        df_imputed_all.columns = df_original.columns #the columns are the same as original because it is a prediction
        ...
        # Imputed All MOFA
        df_imputed_MOFA_all = pd.read_csv("/home/sofiaapolinario/test/dataMOFA/proteomicsMOFA.csv")
        df_imputed_MOFA_all = df_imputed_MOFA_all.rename(columns={'Unnamed: 0': 'index_col'})
        index_proteomics = []
        ...
        for sample in df_imputed_MOFA_all.index:
            ...
            # MOFA and New
            corr_imputed_mofa = corr_sample(sample, df_new_scaled, df_imputed_MOFA_all_scaled)
            corr_imputed_NA_mofa = corr_sample(sample, df_new_scaled, df_imputed_MOFA_na_scaled)
            

            #Append the row
            corr_row = {"cell_line" : sample ,  
        ...
        sns.set_theme()
        sns.set_context("paper")
        ax = sns.scatterplot(data=correlation, x="corr_original", y="corr_imputed", alpha=0.5, color='orange')
        ax.set(title='MSE per sample with New Proteomics Dataset (Autoencoder)', xlabel='MSE per sample (original dataset, new dataset)', ylabel='MSE per sample (decoder output, new dataset)')
        X_plot = np.linspace(-0.2, 5, 20)
        Y_plot = X_plot
        plt.plot(X_plot, Y_plot, color='r')
        indices = np.where(np.isfinite(correlation['corr_original'].values) & np.isfinite(correlation['corr_imputed'].values))[0]
        r, p = stats.pearsonr(x = correlation['corr_original'].iloc[indices].values, y = correlation['corr_imputed'].iloc[indices].values)
        plt.text(.05, .8, 'personr = {:.4f}'.format(r), transform=ax.transAxes)
        plt.text(.05, .75, 'p-value = {:.4f}'.format(p), transform=ax.transAxes)
        plt.savefig(f'/home/sofiaapolinario/test/plots/proteomics/{dt}proteomics_original_imputed_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        sns.set_theme()
        sns.set_context("paper")
        ax = sns.scatterplot(data=correlation, x="corr_original", y="corr_imputed_NA", alpha=0.5, color='orange')
        ax.set(title='MSE per sample with New Proteomics Dataset (Autoencoder)', xlabel='MSE per sample (original dataset, new dataset)', ylabel='MSE per sample (imputed dataset, new dataset)')
        X_plot = np.linspace(-0.2, 5, 20)
        Y_plot = X_plot
        plt.plot(X_plot, Y_plot, color='r')
        indices = np.where(np.isfinite(correlation['corr_original'].values) & np.isfinite(correlation['corr_imputed_NA'].values))[0]
        r, p = stats.pearsonr(x = correlation['corr_original'].iloc[indices].values, y = correlation['corr_imputed_NA'].iloc[indices].values)
        plt.text(.05, .8, 'personr = {:.4f}'.format(r), transform=ax.transAxes)
        plt.text(.05, .75, 'p-value = {:.4f}'.format(p), transform=ax.transAxes)
        plt.savefig(f'/home/sofiaapolinario/test/plots/proteomics/{dt}proteomics_original_imputed_NA_.pdf', dpi=300, bbox_inches="tight") 
        plt.close() 
        ...
        sns.set_theme()
        sns.set_context("paper")
        ax = sns.scatterplot(data=correlation, x="corr_original", y="corr_imputed_mofa", alpha=0.5, color='orange')
        ax.set(title='MSE per sample with New Proteomics Dataset (MOFA)', xlabel='MSE per sample (original dataset, new dataset)', ylabel='MSE per sample (model output, new dataset)')
        X_plot = np.linspace(-0.2, 5, 20)
        Y_plot = X_plot
        plt.plot(X_plot, Y_plot, color='r')
        indices = np.where(np.isfinite(correlation['corr_original'].values) & np.isfinite(correlation['corr_imputed_mofa'].values))[0]
        r, p = stats.pearsonr(x = correlation['corr_original'].iloc[indices].values, y = correlation['corr_imputed_mofa'].iloc[indices].values)
        plt.text(.05, .8, 'personr = {:.4f}'.format(r), transform=ax.transAxes)
        plt.text(.05, .75, 'p-value = {:.4f}'.format(p), transform=ax.transAxes)
        plt.savefig(f'/home/sofiaapolinario/test/plots/proteomics/{dt}proteomics_original_imputed_mofa_.pdf', dpi=300, bbox_inches="tight")
        plt.close() 
        ...
        sns.set_theme()
        sns.set_context("paper")
        ax = sns.scatterplot(data=correlation, x="corr_original", y="corr_imputed_NA_mofa", alpha=0.5, color='orange')
        ax.set(title='MSE per sample with New Proteomics Dataset (MOFA)', xlabel='MSE per sample (original dataset, new dataset)', ylabel='MSE per sample (imputed dataset, new dataset)')
        X_plot = np.linspace(-0.2, 5, 20)
        Y_plot = X_plot
        plt.plot(X_plot, Y_plot, color='r')
        indices = np.where(np.isfinite(correlation['corr_original'].values) & np.isfinite(correlation['corr_imputed_NA_mofa'].values))[0]
        r, p = stats.pearsonr(x = correlation['corr_original'].iloc[indices].values, y = correlation['corr_imputed_NA_mofa'].iloc[indices].values)
        plt.text(.05, .8, 'personr = {:.4f}'.format(r), transform=ax.transAxes)
        plt.text(.05, .75, 'p-value = {:.4f}'.format(p), transform=ax.transAxes)
        plt.savefig(f'/home/sofiaapolinario/test/plots/proteomics/{dt}proteomics_original_imputed_mofa_NA_.pdf', dpi=300, bbox_inches="tight")
        plt.close() 
        
        
        ##-- MOFA VS AutoEncoder --##
        proteomics_corr = pd.merge(proteomics_imputed_all_table, mofa_imputed_all_table,  how='outer', left_on=['id','GeneSymbol'], right_on = ['id','GeneSymbol'])
        proteomics_corr.columns = proteomics_corr.columns.str.replace('value_x', 'AUTOENCODER')
        proteomics_corr.columns = proteomics_corr.columns.str.replace('value_y', 'MOFA')
        
        sns.set()
        sns.set_theme()
        sns.set_context("paper")
        ax = sns.scatterplot(data=proteomics_corr, x="MOFA", y="AUTOENCODER", alpha=0.5, color='orange')
        X_plot = np.linspace(-6, 18, 20)
        Y_plot = X_plot
        plt.plot(X_plot, Y_plot, color='r')
        indices = np.where(np.isfinite(proteomics_corr['MOFA'].values) & np.isfinite(proteomics_corr['AUTOENCODER'].values))[0]
        r, p = stats.pearsonr(x = proteomics_corr['MOFA'].iloc[indices].values, y = proteomics_corr['AUTOENCODER'].iloc[indices].values)
        plt.text(.05, .8, 'personr = {:.4f}'.format(r), transform=ax.transAxes)
        plt.text(.05, .75, 'p-value = {:.4f}'.format(p), transform=ax.transAxes)
        plt.savefig(f'/home/sofiaapolinario/test/plots/proteomics/{dt}proteomics_autoencoder_mofa_.png', dpi=300, bbox_inches="tight")
        plt.close() 


    def predictions(model, hidden_dim, latent_dim, probability, group, learning_rate, n_folds, batch_size):
        ...
        for views in omics_dataloader:
            ...
            latent_space_transcriptomics = pd.concat([latent_space_transcriptomics,pd.DataFrame(views_latent[0])])
            latent_space_proteomics = pd.concat([latent_space_proteomics,pd.DataFrame(views_latent[1])])
            latent_space_drugresponse = pd.concat([latent_space_drugresponse,pd.DataFrame(views_latent[2])])
            latent_space_crisprcas9 = pd.concat([latent_space_crisprcas9,pd.DataFrame(views_latent[3])])
            latent_space_metabolomics = pd.concat([latent_space_metabolomics,pd.DataFrame(views_latent[4])])
            latent_space_methylation = pd.concat([latent_space_methylation,pd.DataFrame(views_latent[5])])
            proteomics_final = pd.concat([proteomics_final,pd.DataFrame(views_hat[1].tolist())])
            drugresponse_final = pd.concat([drugresponse_final,pd.DataFrame(views_hat[2].tolist())])
        ...
        #latent_space.to_csv(f'/home/sofiaapolinario/test/dataoutputs/{dt}latent_space.csv', index=True)
        #latent_space_transcriptomics.to_csv(f'/home/sofiaapolinario/test/dataoutputs/{dt}latent_space_transcriptomics.csv', index=True)
        #latent_space_proteomics.to_csv(f'/home/sofiaapolinario/test/dataoutputs/{dt}latent_space_proteomics.csv', index=True)
        #latent_space_drugresponse.to_csv(f'/home/sofiaapolinario/test/dataoutputs/{dt}latent_space_drugresponse.csv', index=True)
        #latent_space_crisprcas9.to_csv(f'/home/sofiaapolinario/test/dataoutputs/{dt}latent_space_crisprcas9.csv', index=True)
        #latent_space_methylation.to_csv(f'/home/sofiaapolinario/test/dataoutputs/{dt}latent_space_methylation.csv', index=True)
        #latent_space_metabolomics.to_csv(f'/home/sofiaapolinario/test/dataoutputs/{dt}latent_space_metabolomics.csv', index=True)

        plot_latent_spaces(latent_space, latent_space_transcriptomics, latent_space_proteomics, latent_space_drugresponse, latent_space_crisprcas9, latent_space_metabolomics, latent_space_methylation, hidden_dim, latent_dim, probability, group, learning_rate, n_folds, batch_size_initial)
        evaluate_drug_response(drugresponse_final)
        evaluate_proteomics(proteomics_final)
    

    def epoch(data, num_epochs, hidden_dim, latent_dim, probability, group, learning_rate, n_folds, batch_size, alpha_KL, alpha_MSE, optimizer_type, w_decay, loss_type, activation_function):
        ...
        if optimizer_type == "adam":
            ...
            optimizer = torch.optim.RAdam(model.parameters(), lr=learning_rate, weight_decay = w_decay)
        
        losses_dict = {'loss_train': [],'mse_train': [],'kl_train': [],'loss_val': []}
        ...
        for epoch in range(num_epochs):
            ...
            print(f'Epoch {epoch + 1}/{num_epochs} | Loss (train): {losses_dict["loss_train"][epoch]:.4f} | Loss (val): {losses_dict["loss_val"][epoch]:.4f}')
            
            #Early Stopping
            #if epoch > 7 :
            #    if (losses_dict["loss_val"][-7] < losses_dict["loss_val"][-1]) & (losses_dict["loss_val"][-2] < losses_dict["loss_val"][-1]):
            #        break
        
        plot_losses(losses_dict,losses_datasets, alpha_KL, alpha_MSE)
        predictions(model, hidden_dim, latent_dim, probability, group, learning_rate, n_folds, batch_size)
    ...
    epoch(data = omics_db, 
        ...
        probability = 0.6, 
        group = 15, 
        alpha_KL = 0.03707796141402508, 
        alpha_MSE = 0.7066146649015657,
        optimizer_type = "adam",
        w_decay =  1e-05,
        loss_type = "mse",
