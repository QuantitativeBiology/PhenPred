...
class MOVE(nn.Module):
    def __init__(
        ...
        self.views_latent_sizes = {
            k: int(v * self.hypers["latent_dim"]) for k, v in self.views_sizes.items()
        }
    ...
    def _build(self):
        ...
        self.joint = Gaussian(
            latent_views_sum,
            int(latent_views_sum * self.hypers["latent_dim"]),
        )
    ...
    def _build_decoders(self):
        latent_views_sum = int(
            sum(self.views_latent_sizes.values()) * self.hypers["latent_dim"]
        )
