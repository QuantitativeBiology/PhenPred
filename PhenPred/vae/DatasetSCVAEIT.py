...
class CLinesDatasetSCVAEIT:
    ...
    def load_reconstructions(
        ...
        dfs_imputed = {}
        for n in dfs:
            df_file = f"{ddir}/scvaeit_{n}_imputed_{n_factors}factor.csv"
        ...
        joint_latent = dict(
            factors=pd.read_csv(
                f"{ddir}/scvaeit_latent_{n_factors}factor.csv", index_col=0
            )
        )
