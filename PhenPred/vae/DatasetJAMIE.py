...
class CLinesDatasetJAMIE:
    ...
    def load_reconstructions(
        ...
        dfs_imputed = {}
        for n in dfs:
            df_file = f"{ddir}/jamie_imputed_{n}_df.csv"
        ...
        joint_latent = dict(
            factors=pd.read_csv(
                f"{ddir}/jamie_integrated_transcriptomics_drug_df.csv", index_col=0
            )
        )
